# AGENTS.md — Paymos for PrestaShop

PrestaShop payment module for Paymos stablecoin payments. Thin shell over the bundled
...
- Front controllers are reached as `index.php?fc=module&module=paymos&controller={name}`; the module must be installed and enabled for the route to resolve at all.
- Order state transitions go through `validateOrder`/`setCurrentState` so PrestaShop's mail and stock hooks fire — never update the order row directly.
- The module directory name, the main class name, and the `$this->name` value must all match, or PrestaShop refuses to install it.
